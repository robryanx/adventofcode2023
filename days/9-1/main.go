...
func main() {
	...
	for _, row := range rows {
		...
		numbers[currentRow] = append(numbers[currentRow], 0)

		for i := currentRow - 1; i >= 0; i-- {
			newValue := numbers[i+1][len(numbers[i+1])-1]
			newValue += numbers[i][len(numbers[i])-1]

			numbers[i] = append(numbers[i], newValue)
		}

		total += numbers[0][len(numbers[0])-1]
	}
